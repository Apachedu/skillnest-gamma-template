...
async function createGeneration(inputText, title){
  const body = {
    ...
    additionalInstructions: `Title: ${title}. Keep headings concise; do not rewrite user text.`
  };
...
<div class="wrap">
  ...
  ${results.map(r=>`
    <div class="card">
      ...
      <p>
        ${r.gammaUrl ? `<a class="btn" href="${r.gammaUrl}" target="_blank" rel="noopener">Open deck</a>` : `<span>Link pending</span>`}
